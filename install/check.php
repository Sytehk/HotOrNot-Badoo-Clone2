...
$ionCubeLink = (isset($_SERVER['HTTPS']) ? "https" : "http") . "://".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];

$cb = substr($ionCubeLink, -1);
if($cb == '/'){
    $ionCubeLink = $ionCubeLink.'ioncube/loader-wizard.php';   
} else {
    $ionCubeLink = $ionCubeLink.'/ioncube/loader-wizard.php';  
}
...
check(version_compare(phpversion(), '7.1', '>='), sprintf('Checking that PHP version is at least 7.1 (%s installed)', phpversion()), 'Install PHP 7.1 or newer (current version is '.phpversion(), true);
check($ionCubeInstalled, sprintf('Checking if ionCube is available (current version is '.$ionCubeVersion.')'),'ionCube not found, install the ionCube using the <a href="'.$ionCubeLink.'" style="color:#000cff" target="_blank">ionCube Loader Wizard</a>', true);
...
check(extension_loaded("zip"), sprintf('Checking that php_zip support is available'), 'Install the php_zip extension. http://pecl.php.net/package/zip', false);
